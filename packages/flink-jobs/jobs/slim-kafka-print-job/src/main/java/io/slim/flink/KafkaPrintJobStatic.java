...
public class KafkaPrintJobStatic {

    private static Logger log = LoggerFactory.getLogger(KafkaPrintJobStatic.class);
    ...
    private static String loadKafkaPem() {
        try (var inputStream = KafkaPrintJobStatic.class.getClassLoader().getResourceAsStream("kafka.pem")) {
            if (inputStream == null) {
    ...
    private static String resolveKafkaPemPath() {
        try {
            ...
            var resourceUrl = KafkaPrintJobStatic.class.getClassLoader().getResource(candidatePath);
            if (resourceUrl == null) {
